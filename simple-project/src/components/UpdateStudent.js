...
const  UpdateStudent=(props) => {
    ...
    const[studentObj , setStudentObj] = useState({name: props.student.name, gender: props.student.gender ,
        branch:props.student.branch,id: props.student.id , email :props.student.email,password:props.student.password})
    const handleSubmit = (e)=>{
        e.preventDefault();
        console.log(studentObj);
        Axios.put(`http://localhost:5000/student/${studentObj.id}`,studentObj).then((response)=>{
                ...
                <div className="card-body login-card-body">                  
                  ...
                  <div className="input-group mb-3">
                    ...
                    <div className="form-check">
                          {studentObj.gender=== "Male" ? <input className="form-check-input" type="radio" onChange={handleChange} value={"Male"} checked  name="gender" /> : 
                          <input className="form-check-input" type="radio" onChange={handleChange} value={"Male"} checked  name="gender" />
                          }
                          <label className="form-check-label"  >Male</label>
                        </div>
                        <div className="form-check">
                        {   studentObj.gender==="Female" ? <input className="form-check-input" type="radio" onChange={handleChange} checked value={"Female"} name="gender" /> :
